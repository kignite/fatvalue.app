...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#fdf6f0] text-[#4a3a3a] flex flex-col items-center justify-start p-4 md:p-8">
      {/* Header */}
      <div className="w-full max-w-4xl flex items-center justify-between mb-8">
        <div className="flex items-center gap-2">
          <Weight className="text-orange-400" />
          <h1 className="text-xl font-bold text-orange-900 tracking-tight">揭曉妳的肉有多昂貴</h1>
        </div>
        <div className="bg-white/50 px-4 py-1 rounded-full text-sm font-medium border border-orange-100 flex items-center gap-2">
      ...
      {/* Main Container */}
      <main className="w-full max-w-4xl bg-white rounded-3xl shadow-xl overflow-hidden flex flex-col md:flex-row border border-orange-50">
        
        {/* Sidebar / Host UI */}
        <div className="w-full md:w-1/3 bg-[#f8f9fa] p-8 flex flex-col items-center border-b md:border-b-0 md:border-r border-gray-100">
          <div className="w-40 h-40 rounded-3xl overflow-hidden shadow-2xl mb-6 relative border-4 border-white transform hover:scale-105 transition-transform">
          ...
          <div className="text-center space-y-4">
            <h2 className="text-lg font-bold text-gray-800 tracking-wider">成就見證官</h2>
            <div className="bg-white p-5 rounded-2xl text-sm leading-relaxed shadow-sm relative border border-gray-100 italic text-gray-600">
        ...
        {/* Game Content */}
        <div className="flex-grow p-8 flex flex-col items-center justify-center min-h-[500px] relative bg-gradient-to-b from-white to-[#fffcf9]">
          {state.stage === 'INTRO' && (
            <div className="text-center animate-in fade-in zoom-in duration-700">
              <div className="mb-6 inline-block p-5 bg-orange-50 rounded-full shadow-inner">
                <Sparkles className="text-orange-400 w-12 h-12 animate-pulse" />
              </div>
              <h1 className="text-3xl font-black text-gray-800 mb-4 tracking-tight uppercase">妳的肉有多昂貴<br/><span className="text-orange-500 font-bold text-xl tracking-widest">— 2025 減脂價值報告 —</span></h1>
              <p className="text-gray-400 mb-8 max-w-xs mx-auto leading-loose text-sm">
                妳成功告別了 15 公斤脂肪。<br/>這不是體重的減少，而是品質的躍升。<br/>現在，讓我們衡量妳這份意志力的總價值。
              </p>
              <button 
                onClick={startGacha}
                className="group relative px-10 py-5 bg-gray-900 hover:bg-black text-white font-bold rounded-2xl shadow-2xl transition-all hover:scale-105 active:scale-95 flex items-center gap-3 mx-auto tracking-widest text-sm uppercase"
              >
                Enter Evaluation
          ...
          {state.stage === 'DRAWING' && (
            <div className="w-full h-full flex flex-col items-center">
              <div className="flex-grow flex items-center justify-center w-full">
                {currentCard ? (
                  <div key={currentCard.id} className="animate-in zoom-in spin-in-2 duration-700">
                  ...
                  <div className="text-center space-y-6">
                    ...
                    <button 
                      onClick={drawNext}
                      className="px-12 py-5 bg-gray-900 text-white font-black rounded-2xl shadow-xl transition-all hover:bg-black hover:scale-105"
                    >
                      揭曉第一公斤
                    </button>
                  </div>
              ...
              <div className="mt-8 flex flex-col items-center gap-4 h-20">
                {isRevealed && (
                  <div className="animate-in slide-in-from-bottom-4 duration-500 flex flex-col items-center gap-4">
                    {state.results.length < MAX_DRAWS ? (
                      <button 
                        ...
                        className="px-12 py-4 bg-gray-900 hover:bg-black disabled:bg-gray-300 text-white font-black rounded-2xl shadow-xl transition-all hover:scale-105 active:scale-95 flex items-center gap-2 text-sm tracking-widest uppercase"
                      >
                        <Sparkles size={16} />
                      ...
                      <button 
                        onClick={finishGame}
                        className="px-12 py-4 bg-rose-600 hover:bg-rose-700 text-white font-black rounded-2xl shadow-xl transition-all hover:scale-105 active:scale-95 flex items-center gap-2 text-sm tracking-widest uppercase"
                      >
                        Final Valuation
                        <ArrowRight size={16} />
                      </button>
          ...
          {state.stage === 'SUMMARY' && (
            <div className="w-full h-full flex flex-col animate-in fade-in duration-1000">
               <div className="text-center mb-6">
                 <h2 className="text-2xl font-black text-gray-900 mb-2 tracking-tighter uppercase">Achievement Breakdown</h2>
                 <p className="text-gray-400 text-xs tracking-widest font-bold">15KG FAT REDUCTION · 2025 EDITION</p>
               </div>
               ...
               <div className="bg-[#fcfcfc] p-7 rounded-3xl border border-gray-100 mb-8 shadow-inner">
                  ...
                  <div className="text-sm text-gray-500 leading-relaxed italic font-medium">
                    {loadingEvaluation ? (
                      <div className="flex items-center gap-2">
               ...
               <div className="flex flex-col md:flex-row items-center justify-between gap-6 mt-auto">
                  <div className="text-center md:text-left">
                    ...
                    <div className="text-5xl font-black text-gray-900 tracking-tighter">$ {totalValue.toLocaleString()} <span className="text-sm font-bold text-gray-300 ml-1 uppercase">TWD</span></div>
                  </div>
                  <button 
                    onClick={() => { window.location.href = '/retry.html'; }}
                    className="px-8 py-4 border-2 border-gray-200 text-gray-400 hover:text-black hover:border-black font-black rounded-2xl transition-all flex items-center gap-2 text-xs tracking-widest uppercase"
                  >
                    <RotateCcw size={16} />
                    最後一夜的再次評估
                  </button>
      ...
      {/* Footer Info */}
      <footer className="mt-8 text-center text-gray-400 text-[10px] font-bold tracking-[0.3em] uppercase opacity-50">
